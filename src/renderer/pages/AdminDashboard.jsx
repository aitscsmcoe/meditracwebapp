...
export default function AdminDashboard() {
  ...
  const handleActivate = async (doctorId) => {
    ...
    if (doctor.status !== "Requested")
      return alert("Activate allowed only when status is Requested.");

    if (
      !window.confirm(
        "Activate this doctor? Activation date will be set to today and renewal +1 year."
      )
    )
      return;
  ...
  const handleRenew = async (doctorId) => {
    ...
    if (
      !window.confirm(
        "Renew this doctor's subscription? This sets expected renewal to +1 year from today."
      )
    )
      return;
  ...
  // Renew doctor for six months
  const handleRenewsixmonth = async (doctorId) => {
    const activationDate = new Date();
    const doctor = doctors.find((d) => d.id === doctorId);
    if (!doctor) return alert("Doctor not found.");
    ...
    if (
      !window.confirm(
        "Renew this doctor's subscription? This sets expected renewal to +6 month from today."
      )
    )
      return;
  ...
  const handleSuspend = async (doctorId) => {
    ...
    if (
      !window.confirm("Suspend this doctor? They will not be able to log in.")
    )
      return;
  ...
  const handleDelete = async (doctorId) => {
    if (!window.confirm("Delete this doctor record permanently?")) return;
    try {
      const ref = doc(adminDb, "DoctorsRegistered", doctorId);
      await deleteDoc(ref);
  ...
  // NEW: status options and filtered list
  const statusOptions = [
    "All",
    "Registered",
    "Requested",
    ...
    <div style={outer}>
      <div style={header}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
          ...
          <div style={filterRow}>
            <label style={{ marginRight: 6, fontWeight: 600, fontSize: 13 }}>
              Status:
            </label>
            <select
              value={statusFilter}
          ...
          <button onClick={loadDoctors} style={btnSecondary}>
            Refresh
          </button>
                      ...
                      <tr key={d.id} style={row}>
                        ...
                        <td style={td}>
                          ...
                          >
                            ...
                            <button
                              onClick={() => handleRenewsixmonth(d.id)}
                              disabled={!renewEnabled}
                              style={{
                                ...btnRenew,
                                padding: "4px 8px",
        ...
        <div style={modalOverlay}>
          <div style={modalContent}>
            ...
            <div style={{ lineHeight: 1.6 }}>
              ...
              <p>
                <b>Mobile:</b> {selectedDoctor.mobile || "-"}
              </p>
              <p>
                <b>Clinic:</b> {selectedDoctor.clinicName || "-"}
              </p>
              <p>
                <b>Clinic Address:</b> {selectedDoctor.clinicAddress || "-"}
              </p>
              <p>
                <b>Doctor Address:</b> {selectedDoctor.doctorAddress || "-"}
              </p>
              <p>
                <b>Degree:</b> {selectedDoctor.degree || "-"}
              </p>
              <p>
              ...
              <p>
                <b>Status:</b> {selectedDoctor.status || "Registered"}
              </p>
              <p>
                <b>Registered:</b> {formatDate(selectedDoctor.registrationDate)}
              </p>
              <p>
                <b>Activation:</b> {formatDate(selectedDoctor.activationDate)}
              </p>
              <p>
                <b>Renewal:</b> {formatDate(selectedDoctor.expectedRenewalDate)}
              </p>
            </div>
